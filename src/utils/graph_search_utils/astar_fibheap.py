...
class AStarNode:
    ...
    def __init__(self, f=0, h=0, g=0, nid=0, state=None, parent=None, parentedge=None, children=None):
        self.f = f
        self.h = h
        self.g = g
        self.nid = nid
...
class AStar:
    ...
    def search_step(self):
        ...
        # Check goal condition (only if testgoalongeneration is false)
        ...
        for succ, cost, edge in zip(successors, costs, edges):
            ...
            if self.testGoalOnGeneration and self.is_goal(succ):
                # Note: this was motified to fix a bug whereby self.goal=succ(n)_i.state, not succ(n)_i | temp fix: self.goal=n
                self.goal = n
                return True
            ...
            if visited:
                ...
                # adjust its value in the fringe
                    ...
                    # Note: this was motified to fix a bug where removing from fringe causes index error

                    # First, remove the prior instance from the fringe
                    # Implementation has bug
                    # if hasattr(visited, 'heapNode'):
                    #     try:
                    #         print(visited.heapNode)
                    #         print(visited.heapNode.key)
                    #         self.fringe.delete(visited.heapNode)
                    #     except Exception as e:
                    #         print("Deleting from fringe failed. Heuristic inadmissible or inconsistent?")
                    #         raise
