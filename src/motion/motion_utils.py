...
class MotionUtils:
    ...
    def __init__(
            self, ProjectConstants, world, height_map: HeightMap, scatter_list, state_path, gradient_map, u_input=None,
        ...
        self.bl_end_effector = self.robosimian.link(self.ProjectConstants.BL_ACTIVE_DOFS[len(self.ProjectConstants.BL_ACTIVE_DOFS) - 1])
        self.br_end_effector = self.robosimian.link(self.ProjectConstants.BR_ACTIVE_DOFS[len(self.ProjectConstants.BR_ACTIVE_DOFS) - 1])
        self.fl_end_effector = self.robosimian.link(self.ProjectConstants.FL_ACTIVE_DOFS[len(self.ProjectConstants.FL_ACTIVE_DOFS) - 1])
        self.fr_end_effector = self.robosimian.link(self.ProjectConstants.FR_ACTIVE_DOFS[len(self.ProjectConstants.FR_ACTIVE_DOFS) - 1])
        self.torso = self.robosimian.link(self.ProjectConstants.TORSO_LINK_INDEX)
        self.height_map = height_map
        if include_ik_solver:
            from .ik_solver_utils import IKSolverUtils
            self.collider = WorldCollider(self.world)
            self.IKSolverUtil = IKSolverUtils(ProjectConstants, world, height_map, self.scatter_list, self.stance_path, gradient_map)
    ...
    def get_active_dofs_from_end_effector(self, end_effector):
        if end_effector == 1:
            return self.ProjectConstants.FL_ACTIVE_DOFS
        elif end_effector == 2:
            return self.ProjectConstants.FR_ACTIVE_DOFS
        elif end_effector == 3:
            return self.ProjectConstants.BR_ACTIVE_DOFS
        elif end_effector == 4:
            return self.ProjectConstants.BL_ACTIVE_DOFS
        else:
            msg = "Error finding: "+str(end_effector)
            Logger.log(msg, "FAIL")
            return False
    ...
    def get_shoulder_from_end_effector(self, end_effector):
        if not end_effector in [1, 2, 3, 4]:
            ...
            return
        if end_effector == 4:
            end_effector = self.robosimian.link(self.ProjectConstants.BL_ACTIVE_DOFS[0])
        elif end_effector == 3:
            end_effector = self.robosimian.link(self.ProjectConstants.BR_ACTIVE_DOFS[0])
        elif end_effector == 1:
            end_effector = self.robosimian.link(self.ProjectConstants.FL_ACTIVE_DOFS[0])
        else:
            end_effector = self.robosimian.link(self.ProjectConstants.FR_ACTIVE_DOFS[0])
        return end_effector
    ...
    def get_end_effector_rotation_matrix(self, end_effector, xyz=None, yaw_rad=None, visualize_normal=False, debug=False):
        ...
        if end_effector in [1, 4]:
            upright = [0, 0, -1, 0, -1, 0, -1, 0, 0]
        if xyz:
            xyz[2] -= self.ProjectConstants.END_EFFECTOR_HEIGHT
            x_grad, y_grad = self.gradient_map.get_grad_at_world_xy(xyz[0], xyz[1])
            x_grad /= 2*self.ProjectConstants.HM_X_GRANULARITY; y_grad /= 2*self.ProjectConstants.HM_Y_GRANULARITY
    ...
    def get_torso_z_des_from_xy(self, torso_xy, curr_state, debug=False):
        ...
        min_torso_clearance = self.ProjectConstants.MIN_TORSO_CLEARANCE
        torso_z_des = self.ProjectConstants.TORSO_Z_DESIRED
    ...
    def adjust_endeff_z(self, xyz):
        offset = self.ProjectConstants.END_EFFECTOR_HEIGHT
    ...
    def get_support_triangle_from_points(self, P, diag_points, name=None):
        ...
        support_tri.enforce_safety_margin(self.ProjectConstants.SUPPORT_TRIANGLE_SAFETY_MARGIN)
        return support_tri
    ...
    def get_torso_range_from_end_effector(self, end_effector, at_point=None, yaw_rads=None):
        R = self.ProjectConstants.END_AFFECTOR_RADIUS_TO_SHOULDER
        S = self.ProjectConstants.SHOULDER_TORSO_XY_EUCLIDEAN_DIF
        yaw = self.get_current_torso_yaw_rads()
        if yaw_rads: yaw = yaw_rads
        psi = self.ProjectConstants.SHOULDER_TORSO_PSI_RADS
        ...
        return self.ProjectConstants.END_RANGE_MULTIPLIER * r
