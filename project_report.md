...
of the robots end effector plus a safety margin (config.END_AFFECTOR_RADIUS + config.COST_MAP_SEARCH_MARGIN_WIDTH)
at the specified $`(x,y)`$ coordinate. If the x or y slope is greater than a specified maximum allowable slope, stored as  
config.CMAP_MAX_NONPENALIZED_SLOPE, the function returns a cost linearly proportional to the difference of the 
calculated slope and the max allowable slope. If the slope is greater than a predefined value, stored as 
config.CMAP_SLOPE_HEURISTIC_MAX_CONSIDERED_DEGREE, it is presumed to be as a result of a large height discrepancy 
...
the cost map. Each $`(x,y)`$ scatter point is spaced by a predetermined granularity (config.(X/Y)_STEP_SIZE_FS_SCATTER) 
...
base state, defined by config.END_EFFECTOR_DELTA_X/Y_FROM_TORSO at the `x,y` coordinates specified by the state 
...
weights, respectivly. These variables are set in config.HL_TRAJ_(G/H)_WEIGHT. Reasonable values were found to 
...
Get-Next-Leg-To-Move($`leg_{current}`$) returns the next leg to move in the step order. The provided step order is \[3,2,4,1\], defined by config.STEP_ORDER, indicating that
...
To evaulate the cost of a state, the planner creates a hybrid cost map, created by adding a parobola of slope config.STEP_SEQ_COST_PARABOLA_SLOPE centered at 
...
solved config.END_CONFIG_SAMPLE_COUNT times, whereas in each iteration the torso is randomly offset by x,y,z $`\in \(-.025,.025,)`$.
